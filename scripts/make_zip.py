...
def  main():
  ...
  installer = "\installer\ComPressure Installer.exe"
   
  if demo:
    installer = "\installer\ComPressure Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\ComPressure manual.pdf" 
  ]
  ...
  ZIPNAME = "ComPressure-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "ComPressure-v" + FULLVERSIONSTR + "-win-demo.zip"
